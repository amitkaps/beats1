...
# Remove trailing and after whitespace from song and artist name
df$song <- str_trim(df$song)
df$artist <- str_trim(df$artist)

# Create clean song-artist combo
...
Lets now save the data a smaller data frame of what we want - created, song, artist, and url.

```{r}
# Save a smaller data frame
df.small <- df.full %>%
            select(created, song, artist, url)
